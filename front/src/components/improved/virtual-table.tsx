...
export type BasicColumnData = { id: number | string };

export type Column<T extends BasicColumnData> = { header?: string } &
...
// steps:
// data is provided by redux store
// stoer can contain 100000 items, but the table wil show only 200
// if we get to the limit of table we load more (to the store)
// if se go up/down, the table gets the necessary items from the store
// mb tabke has 3 zones, hwile in th emiddle one you are safe nothing is done
// if you go to any of those near then lo
...
function Table<T extends BasicColumnData>(
    {
        columns,
        data,
        loadMore,
